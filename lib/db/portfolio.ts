 ...
 * @param portfolioItems Array of portfolio items to add
...
export async function addMultiplePortfolioEntries(
  ...
  try {
    ...
    for (const item of portfolioItems) {
      ...
      if (existingEntry) {
        throw new Error(
          `An entry with timestamp ${item.createdAt} already exists`
        );
      }
    }
    ...
    if (
      !result.acknowledged ||
      result.insertedCount !== portfolioItems.length
    ) {
      throw new Error("Failed to add all portfolio entries");
    }
